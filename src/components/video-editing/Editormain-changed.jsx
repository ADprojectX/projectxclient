...
import { Construction, Widgets } from '@mui/icons-material';
...
const Editormain = ({ scenes }) => {
    ...
    const handleMetadataLoaded = () => {
        ...
        setVideoDurations(prevDurations => [...prevDurations, videoDuration]);
    ...
    useEffect(() => {
      console.log("useeffect - calc seconds")

      async function fetchBlobDuration() {
        const blobUrl = scenes[0];
        try {
          const duration = await getBlobDuration(blobUrl);
          console.log(duration + ' seconds');
        } catch (error) {
          console.error('Error fetching blob duration:', error);
        }
      }
    ...
    useEffect(() => {
      console.log("useeffect- videodurations")

        const tempTotalDuration = videoDurations.reduce((acc, curr) => acc + curr, 0);
        let tempStartTimes = [];
        let accumulatedDuration = 0;
        videoDurations.forEach((duration, index) => {
            tempStartTimes.push(accumulatedDuration);
    ...
    const handleSceneEnd = () => {
      console.log("handlesceneend")

        const nextSceneIndex = currentSceneIndex + 1;
        if (nextSceneIndex < scenes.length) {
            videoRef.current.src = scenes[nextSceneIndex];
    ...
    return (
        <div className='editor-main'>
            ...
            <div className='v-player grid-col-span-2'>
                <VPlayer 
                    videoSrc={scenes[currentSceneIndex]} 
